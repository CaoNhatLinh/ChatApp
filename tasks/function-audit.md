...
| Backend compile/test | Working at unit level | With Java 20 (`JAVA_HOME=C:\\Program Files\\Java\\jdk-20`), `mvnw test` reports 155 tests, 0 failures, 0 errors. The host default Java 17 cannot run the Java 20 test classes; clean-stack integration is still pending. |
...
935 static Vietnamese copy keys are checked and currently have no missing
...
| Room roles/membership | Custom roles/color/order/permission union, add/join/assign/remove/kick/leave/transfer owner | Implemented but not fully verified | Add/remove/leave and direct-invite joins enforce duplicate and capacity state with one-partition conditional batches; retry repairs user/admin projections without duplicate audit. Clean Cassandra contention, hierarchy, system-message and owner-transfer proof remain. |
...
| Message interactions | Reply, mention, reaction add/remove/counts, read watermark/seen-by, forward | Implemented but not fully verified | Reactions, monotonic read receipt, revision/pin routes and realtime events are wired; the seen-by dialog now uses a bounded cursor page with hydrated profiles, retry and load-more states; reply/mention/forward and live persistence/reconciliation remain. |
...
| Invites/QR | Preview/create/list/consume/decline/revoke, approval requests, expiry/usage states, QR | Partial | Direct consumption now preflights room capacity, compensates a newly owned use after a lost membership race, and distinguishes concurrent accepted claims; approval concurrency, security and browser persistence evidence remain. |
...
| Accessibility/performance | Loading/empty/error/offline states, keyboard/SR, responsive UI, virtualization, bundle limits | Partial | Public responsive/UI-quality coverage passes; audited workspace controls declare button types and accessible names, and the Contacts segmented control exposes its selected state; full authenticated accessibility tree coverage and bundle-size remediation remain. |
