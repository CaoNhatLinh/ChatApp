...
| Regression safety | Pass for available gates | Java suite 147 tests/0 failures; frontend type-check/lint pass after scoped protocol changes | Live Redis/STOMP multi-node proof remains external | Keep live-stack gaps explicitly documented |
...
| Review dimension | Result | Evidence | Remaining gap | Correction / decision |
| --- | --- | --- | --- | --- |
| Bounded directory loading | Pass at browser boundary | `SidebarConversationList` has a bounded `min-h-0`/`h-full` scroll root; the room-management smoke renders 30 first-page rows, scrolls the recovery action into view and observes exactly the opaque `conversation-cursor-1` request before rendering the second page | Virtualized rendering for extremely large local lists remains a future performance task | Keep page loading tied to the visible scroll window; never fetch the complete directory on mount |
...
| Regression safety | Pass for available gates | `npm run validate`, `npm run build`, and room-management smoke report zero cursor requests before scrolling and exactly one `conversation-cursor-1` request afterward, with zero console/request failures and no horizontal overflow | Clean backend pagination remains pending | Treat browser evidence as UI contract proof, not provider persistence proof |
...
| Test traceability | Partial | Java 20 suite reports 147 tests/0 failures/0 errors; `PresenceServiceTest`, frontend validation and `test:e2e:room-management` cover member pagination plus the sidebar's real scroll-triggered opaque-cursor request | Live multi-node presence evidence remains pending | Add targeted integration proof instead of broadening current claims |
...
# Follow-up self-review (2026-08-30, global operator journey)

| Review dimension | Result | Evidence | Remaining gap | Correction / decision |
| --- | --- | --- | --- | --- |
| Global scope and contract | Pass at HTTP/browser boundary | One authenticated APP_ADMIN journey performs global room policy/archive/restore, report resolution, sanction impose/revoke, user role/status/session/device commands and audit export; exact payloads, reasons, methods and paths are asserted | Clean Cassandra persistence and multi-operator concurrency remain pending | Keep global app authority separate from conversation-local roles |
| Accessibility | Pass for populated admin journey | All visible admin fields have explicit labels; target-specific session/device revoke names remove ambiguous controls; 390px audit reports zero unnamed controls, zero unlabelled fields and no overflow | Manual screen-reader and axe review remain release work | A visible label is not sufficient unless it is programmatically associated |
| Information density | Pass for corrected admin copy | Removed user-facing Cassandra partition, projection/backfill, bounded-query, security-version and token/server jargon; copy now states only the selectable range and action consequence | Message investigation still requires a bucket identifier from an upstream report/context | Expose operational constraints, not storage implementation details |
| Failure and recovery | Pass for exercised destructive commands | Archive, restore, report resolution, sanction revoke, suspension, session revoke and device revoke require reasons where applicable and use named confirmation dialogs before authoritative state updates | Provider/live persistence failures are not covered by this success fixture | Preserve prior state until the server mutation succeeds |
| Regression safety | Pass for available gates | `npm run validate`, `npm run test:i18n:copy` (888 keys), production build, `test:e2e:admin` and `test:e2e:ui-quality` pass with zero unexpected console/request failures | Live backend/browser integration remains pending | Treat HTTP-boundary proof as UI-contract evidence, not persistence evidence |

# Follow-up self-review (2026-08-30, virtual member directory and presence batching)

| Review dimension | Result | Evidence | Remaining gap | Correction / decision |
| --- | --- | --- | --- | --- |
| Directory scale | Pass at production-browser boundary | Room management requests 50-member cursor pages; `RoomMemberDirectory` virtualizes variable-height disclosure rows and `test:e2e:room-management` asserts rendered rows remain below loaded member state | Provider-backed extreme-directory profiling remains pending | Retain cursor pages as durable client state while bounding only the DOM window |
| Presence network cost | Pass at deterministic client boundary | `presenceCommandBatcher` coalesces commands for 50 ms by authorization scope, removes duplicate IDs and cancels enter/leave plus unsubscribe/resubscribe pairs; `test:presence:batcher` covers four focused behaviors | Live STOMP frame-volume measurement remains pending | Batch viewport churn without widening the 200-target server authorization boundary |
| Information density | Pass for member rows | A collapsed row exposes avatar, name, presence and at most one custom role plus a compact remainder count; policy and moderation controls remain behind the member disclosure | Manual screen-reader review of virtual row remounts remains pending | Keep data available to authorized operators without rendering all detail by default |
| Pagination recovery | Pass | The measured virtual footer retains the explicit localized load-more action, automatic range loading shares the in-flight guard, and an appended page preserves scroll position instead of cascading through every remaining page | Clean Cassandra cursor ordering remains pending | Require continued scroll intent for each additional page |
| Regression safety | Pass for available gates | Frontend validate, i18n registry, production build, presence tracker/batcher tests and room-management Playwright journey pass with zero console/request failures and no 390px overflow | Clean Redis/Cassandra integration remains pending | Keep browser virtualization evidence distinct from provider persistence evidence |

# Follow-up self-review (2026-08-30, safe message deletion)

| Review dimension | Result | Evidence | Remaining gap | Correction / decision |
| --- | --- | --- | --- | --- |
| Destructive-action safety | Pass at browser boundary | `ChatWindow` opens `ConfirmDialog`; `test:e2e:message-delete` cancels once and observes zero DELETE requests before the explicit confirmation | Live authorization and persistence failure behavior remain pending | Never mutate from the message menu itself; keep the dialog open and preserve state when deletion fails |
| Privacy and tombstone rendering | Pass | The browser fixture deliberately returns a deleted message containing its previous private text; `MessageItem` renders only the localized tombstone and removes attachments, polls and all post-delete actions | Provider-side content retention policy remains a backend concern | Treat `isDeleted` as an authoritative display boundary even if transport data still contains historical fields |
| Locale and accessibility | Pass for exercised flow | The same open session switches from Vietnamese to English; dialog name, description, cancel/confirm actions and the message menu's accessible name update without reload | Manual screen-reader timing review remains pending | Components exposing runtime-localized accessible names must subscribe to locale state |
| Test traceability | Pass for available gates | `ID-MESSAGE-002`, the traceability matrix and `TESTING.md` link the exact bucket-scoped DELETE journey to `test:e2e:message-delete` | Edit, reaction, read-receipt and pin browser coverage plus persisted interaction hydration remain pending | Keep the feature partially verified until every independent message interaction has refresh-safe evidence |

# Follow-up self-review (2026-08-30, draft-preserving message editing)

| Review dimension | Result | Evidence | Remaining gap | Correction / decision |
| --- | --- | --- | --- | --- |
| User-work preservation | Pass at browser boundary | `MessageInput` snapshots text and selected files before edit mode; `test:e2e:message-edit` proves both cancel and successful save restore the unsent text draft | Per-conversation draft persistence across a full reload remains separate work | Let edit mode temporarily own the composer without destroying unrelated user work |
| Information density | Pass for changed surface | The edit banner now identifies the mode and exposes one cancel action without repeating the same message content already present in the textarea and message row; irrelevant attach, poll and call tools are hidden during editing | Full authenticated mobile density review remains broader work | Keep only emoji and save available while editing |
| Touch and accessibility | Pass at component/browser boundary | Composer icon buttons have explicit accessible names; cancel/save labels switch VI→EN in-session; message reaction/menu controls stay visible when hover is unavailable and remain hover/focus-disclosed on pointer devices | Manual screen-reader and physical touch-device review remain pending | Never make a required action hover-only |
| Contract and history correctness | Pass | The browser asserts one exact bucket-scoped PUT with the edited content, renders the canonical `editedAt` marker and opens one bucket-scoped revision-history request containing the previous version | Live persistence and concurrent edit conflict behavior remain pending | Do not infer edit state from client time or a generic `updatedAt` field |
| Test traceability | Pass for available gates | `ID-MESSAGE-002`, `TESTING.md` and the traceability matrix reference `test:e2e:message-edit`; validate, i18n registry, production build and the production-browser journey pass | Reaction/read/pin and refresh hydration remain pending | Keep MESSAGE-002 partially verified |

# Follow-up self-review (2026-08-30, bounded message interaction hydration)

| Review dimension | Result | Evidence | Remaining gap | Correction / decision |
| --- | --- | --- | --- | --- |
| Refresh correctness | Pass at contract/browser boundary | Required `MessagePage.interactions` hydrates reaction count, current-actor reaction and latest-read state; the browser reload proves the updated projection is rendered without a STOMP event | Clean Cassandra restart/recovery remains pending | Interaction UI must be reconstructable from the cursor page, not only live memory |
| Query scale | Pass by implementation and service test | `listMessageHistory` groups only returned page IDs by bucket; each bucket performs one `IN`-bounded reaction read and one read-summary read; service test verifies hydration is requested only for the selected page item | Provider-backed latency/load measurement remains pending | Reject per-message and room-wide interaction reads |
| Write recovery | Pass by idempotent design | Reaction authority uses per-message LWT while the bucket projection is an idempotent row; a retry always repairs projection presence/absence; read progress/receipt/summary share one three-statement logged batch | Fault-injected Cassandra integration proof remains pending | Counter projection was removed because replay cannot safely repair an interrupted counter update |
| Information density and accessibility | Pass for exercised populated state | Only compact badges, seen state and one contextual pin/unpin label render; reaction controls expose localized names; runtime test found and corrected metadata/reaction overlap | Physical touch and manual screen-reader review remain pending | Preserve progressive disclosure and never let invisible metadata intercept a primary action |
| Test traceability | Pass for available gates | ADR 0009, `FLOW-MESSAGE-002`, inventory, OpenAPI and traceability link the Java suite and `test:e2e:message-interactions`; edit/delete browser regressions pass | Live Cassandra/STOMP multi-session proof remains pending | Keep distributed verification separate from HTTP/browser contract proof |

# Follow-up self-review (2026-08-30, scoped message drafts)

| Review dimension | Result | Evidence | Remaining gap | Correction / decision |
| --- | --- | --- | --- | --- |
| User-work preservation | Pass at production-browser boundary | `test:e2e:message-drafts` proves separate Alpha/Beta drafts, full reload, expected 503 preservation and cleanup only after the second successful POST | Server-synchronized multi-device drafts are not implemented | Device-local scope solves refresh/room-switch loss without inventing a backend draft authority |
| Privacy and boundaries | Pass for implemented scope | Storage keys include authenticated user plus conversation; a second account renders empty; edit content and files never enter persisted storage; invalid version 0 is deleted | A shared unlocked browser profile can inspect local data outside product UI | Document local-data scope honestly; do not store access tokens or claim encryption |
| Boundedness and contract | Pass | Exact schema version 1, 50-room cap, 30-day expiry and the canonical 20,000-character limit are enforced; the test asserts the textarea contract | Storage quota/denial is only diagnostic-warning verified | Keep local persistence optional to sending and never truncate user text silently |
| Information density and responsive behavior | Pass | No draft badge, helper paragraph or storage status was added; 390px journey has no horizontal overflow | Physical touch review remains pending | Recovery should be quiet and automatic unless an action fails |
| Regression safety | Pass for available gates | Frontend validate/build, `test:e2e:message-drafts` and `test:e2e:message-edit` pass without unexpected console/request failures | Live backend message durability remains pending | Keep device draft evidence separate from Cassandra send evidence |

# Follow-up self-review (2026-08-30, bounded seen-by receipts)

| Review dimension | Result | Evidence | Remaining gap | Correction / decision |
| --- | --- | --- | --- | --- |
| Contract and privacy | Pass at service/browser boundary | The endpoint returns a bounded `MessageReadReceiptPage` with reader UUID, read time and explicitly allowed profile fields; no email or private account fields cross the boundary; member and message existence are checked before reading | Live Cassandra authorization/persistence proof remains pending | Keep seen-by profile hydration server-owned and do not expose raw IDs as display text |
| Query scale | Pass by implementation and service test | One bounded receipt partition read uses `limit + 1`; one `users_by_id IN` lookup hydrates the returned page; UUID cursor advances from the last returned reader | Provider-backed latency/load measurement remains pending | Reject unbounded room-wide reads and client-side per-reader profile requests |
| Recovery and progressive disclosure | Pass at browser boundary | `MessageReadReceiptDialog` has loading, empty, retry, load-more and close states; `test:e2e:message-interactions` asserts both pages and preserves the message row | Live transport retry and multi-session consistency remain pending | Keep the message page stable while the detail dialog loads |
| Locale and responsive behavior | Pass for exercised flow | Dialog copy and accessible names use the runtime locale; existing full-screen mobile dialog behavior is reused; the smoke switches VI→EN after the interaction flow | Manual screen-reader and physical-device review remain pending | Show seen-by details only on explicit action, not in every message row |
| Regression safety | Pass for available gates | Java suite, frontend validate, i18n registry, build and message-interaction smoke pass with no unexpected console/request failures | Clean Cassandra/STOMP integration remains pending | Keep HTTP/browser evidence distinct from provider-backed evidence |
