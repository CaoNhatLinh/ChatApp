...
| ID-NOTIFY-001 | Notifications and notification settings | Implemented but not fully verified | canonical inbox plus bilingual channel/quiet-hour settings UI, room defaults, personal overrides and explicit precedence endpoint; canonical controller scans the latest 12 monthly partitions; live Cassandra pagination pending |
...
| ID-ADMIN-001 | Global admin workspace (whole-app capabilities, user search, app-role grant/revoke, status, bounded session/device revoke, runtime health, all-room directory/policy/archive, monthly audit timeline) | Implemented but not fully verified | `/admin` UI + `/api/admin/overview` + `/api/admin/audit` + `/api/admin/conversations/**` + `/api/admin/users/{id}/app-roles|status|sessions|devices`; authenticated operator journey and room-directory backfill pending |
...
| ID-ADMIN-002 | Admin moderation/analytics/investigation | Partially implemented | report queue/resolution, APP/CONVERSATION sanctions, bounded daily analytics and reasoned message/revision inspection are implemented; language moderation, appeals, metric dashboards and bounded export remain planned |
