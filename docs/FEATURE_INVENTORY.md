...
| ID-ROOM-001 | DM/group/private-channel/community-channel creation and member management | Implemented but not fully verified | The create-room UI sends explicit visibility and join policy; `test:e2e:room-create` verifies a `CHANNEL` + `COMMUNITY` + `REQUEST_APPROVAL` payload and stable empty-room rendering; live Cassandra and community directory remain pending |
