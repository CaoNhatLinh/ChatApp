...
| Manage a room | Room info/settings/member actions | `/conversations/{id}/members`, `/roles`, `/chat-policy`, `/leave` | `CanonicalConversationController`, `ConversationRoleService` | frontend type-check; live authorization pending |
...
| Investigate analytics/exports | Admin analytics panel and planned investigation tabs | `GET /admin/analytics`, bounded export contracts | `AdminAnalyticsController`, `AdminAnalyticsService`, planned investigation modules | bounded aggregate unit evidence; metric dictionary, live analytics and export remain pending |
