...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  // Fetch real friendship & block status when modal opens
  useEffect(() => {
    ...
    const fetchRelationship = async () => {
      try {
        ...
        // Check friendship via friends list
        if (currentUser?.userId) {
          const friendsResponse = await friendApi.getUsersByStatus(currentUser.userId, 'ACCEPTED');
          const isFriendResult = friendsResponse?.userDetails?.some(
            (f: { userId: string }) => f.userId === userId
          ) ?? false;
          if (isMounted) setIsFriend(isFriendResult);
        }
      } catch (error) {
  ...
  }, [isOpen, userId, isCurrentUser, currentUser?.userId]);
  ...
  const handleBlock = useCallback(async () => {
    ...
    try {
      if (currentUser?.userId) {
        await blockFriend(currentUser.userId, userId);
  ...
  const handleUnblock = useCallback(async () => {
    try {
      if (currentUser?.userId) {
        await unblockFriend(currentUser.userId, userId);
