...
const AdminPage = ({ onBackToApp }: AdminPageProps) => {
    ...
    return (
      <AppPageShell title="Quản trị">
        <Card className="mx-auto mt-12 max-w-xl">
          <CardHeader>
            <CardTitle>{denied ? "Bạn không có quyền truy cập" : "Không tải được khu vực quản trị"}</CardTitle>
            <CardDescription>{errorMessage ?? "Vui lòng thử lại sau."}</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={() => (onBackToApp ? onBackToApp() : router.push("/app"))}><ArrowLeft size={16} /> Về ứng dụng</Button>
            <Button onClick={() => void loadOverview(true)} loading={refreshing}><RefreshCcw size={16} /> Thử lại</Button>
          </CardContent>
        </Card>
      </AppPageShell>
    );
  ...
  const healthState = health?.cassandra?.startsWith("UP") ? "Ổn định" : health ? "Cần kiểm tra" : "Chưa đọc được";
    ...
    <AppPageShell title="Quản trị">
      ...
      >
        <div className="flex flex-wrap items-end justify-between gap-6 border-b border-border pb-8">
          <div>
            <p className="page-kicker">{localizeText("Global admin")}</p>
        ...
        <div className="grid gap-0 border-y border-border md:grid-cols-3">
          ...
          <Card>
            <CardContent className="flex items-start justify-between gap-3 border-0 p-5"><div><p className="text-xs font-semibold text-muted-foreground">Runtime</p><p className="mt-2 text-2xl font-bold">{healthState}</p></div><Server className="text-primary" /></CardContent>
          </Card>
        </div>

          <SurfacePanel title={localizeText("Quản lý toàn bộ room")}>
          {!canReadRooms ? <p className="text-sm leading-6 text-muted-foreground">Role hiện tại chưa có ROOM_READ. Đây là quyền app-level, khác hoàn toàn với role admin trong từng room.</p> : <>
            <div className="flex flex-wrap items-end gap-3">
              <div className="min-w-[170px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">Tháng tạo room (UTC)</label><Input className="mt-1" type="month" value={roomMonth} onChange={(event) => setRoomMonth(event.target.value)} /></div>
              <p className="pb-2 text-xs text-muted-foreground">Danh sách được phân vùng theo tháng để không scan Cassandra vô hạn.</p>
            </div>
            <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.9fr)]">
              <div className="max-h-[360px] space-y-2 overflow-auto pr-1" aria-live="polite">
                {roomsLoading ? <div className="flex justify-center py-8"><LoadingSpinner /></div> : rooms.length ? rooms.map((room) => <button key={room.conversationId} type="button" onClick={() => void selectRoom(room)} className={`flex w-full items-center justify-between gap-3 rounded-xl border px-3 py-3 text-left transition-[color,background-color,border-color,box-shadow,transform,opacity] ${selectedRoom?.conversationId === room.conversationId ? "border-primary/50 bg-primary/10" : "border-border/60 bg-background/55 hover:border-primary/35"}`}><span className="flex min-w-0 items-center gap-3"><Building2 size={17} className="shrink-0 text-primary" /><span className="min-w-0"><span className="block truncate text-sm font-semibold">{room.name ?? room.conversationId}</span><span className="block truncate text-xs text-muted-foreground">{room.conversationType} · {room.memberCount} thành viên · {room.chatMode}</span></span></span><Badge variant={room.deleted ? "destructive" : "outline"}>{room.deleted ? "ARCHIVED" : room.visibility}</Badge></button>) : <p className="rounded-xl border border-dashed border-border/70 px-4 py-8 text-center text-sm text-muted-foreground">Không có room trong tháng này hoặc projection chưa được backfill.</p>}
              </div>
              {selectedRoom ? <div className="rounded-xl border border-border/60 bg-background/45 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{selectedRoom.name ?? selectedRoom.conversationId}</p><p className="mt-1 text-xs text-muted-foreground">{selectedRoom.conversationId}</p></div><Badge variant={selectedRoom.deleted ? "destructive" : "secondary"}>{selectedRoom.deleted ? "ARCHIVED" : "ACTIVE"}</Badge></div><dl className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-muted-foreground">Type</dt><dd className="mt-1 font-semibold">{selectedRoom.conversationType}</dd></div><div><dt className="text-muted-foreground">Members</dt><dd className="mt-1 font-semibold">{selectedRoom.memberCount}</dd></div><div><dt className="text-muted-foreground">Owner</dt><dd className="mt-1 truncate font-semibold">{selectedRoom.ownerId ?? "Chưa có owner"}</dd></div><div><dt className="text-muted-foreground">Created</dt><dd className="mt-1 font-semibold">{formatDate(selectedRoom.createdAt)}</dd></div></dl><div className="mt-5"><p className="text-xs font-semibold text-muted-foreground">Member snapshot</p><div className="mt-2 flex max-h-20 flex-wrap gap-1.5 overflow-auto">{selectedRoom.members.map((member) => <Badge key={member.userId} variant="outline">{member.userId.slice(0, 8)}…</Badge>)}</div></div><label className="mt-5 block text-xs font-semibold text-muted-foreground">Global chat policy</label><select value={roomPolicy} onChange={(event) => setRoomPolicy(event.target.value)} disabled={!canModerateRooms} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm"><option value="OPEN">OPEN</option><option value="READ_ONLY">READ_ONLY</option><option value="MANAGERS_ONLY">MANAGERS_ONLY</option></select><label className="mt-3 block text-xs font-semibold text-muted-foreground">Slow mode (giây)</label><Input className="mt-1" type="number" min={0} max={86400} value={roomSlowMode} onChange={(event) => setRoomSlowMode(Math.max(0, Number(event.target.value) || 0))} disabled={!canModerateRooms} /><label className="mt-3 block text-xs font-semibold text-muted-foreground">Lý do audit</label><Input className="mt-1" value={roomReason} onChange={(event) => setRoomReason(event.target.value)} placeholder="Ví dụ: xử lý report spam" disabled={!canModerateRooms} /><div className="mt-4 flex flex-wrap gap-2"><Button onClick={() => void handleRoomPolicy()} loading={roomMutating} disabled={!canModerateRooms || !roomReason.trim()}>Lưu policy</Button><Button variant={selectedRoom.deleted ? "outline" : "destructive"} onClick={() => void handleRoomArchive()} loading={roomMutating} disabled={!canModerateRooms || !roomReason.trim()}><Archive size={16} />{selectedRoom.deleted ? "Khôi phục" : "Archive room"}</Button></div></div> : <div className="flex min-h-[260px] items-center justify-center rounded-xl border border-dashed border-border/70 px-4 text-center text-sm text-muted-foreground">Chọn một room để xem members và thao tác moderation toàn cục.</div>}
            </div>
          </>}
        </SurfacePanel>

        <SurfacePanel title={localizeText("Audit timeline toàn ứng dụng")}>
          {!canReadAudit ? <p className="text-sm leading-6 text-muted-foreground">Role hiện tại chưa có AUDIT_READ. Audit chỉ hiển thị cho operator được cấp quyền điều tra.</p> : <><div className="flex flex-wrap items-end gap-3"><div className="min-w-[170px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">Tháng audit (UTC)</label><Input className="mt-1" type="month" value={auditMonth} onChange={(event) => setAuditMonth(event.target.value)} /></div><p className="pb-2 text-xs text-muted-foreground">Chỉ đọc tối đa 50 event trong partition tháng.</p></div>{auditLoading ? <div className="flex justify-center py-8"><LoadingSpinner /></div> : auditEvents.length ? <div className="mt-4 max-h-[360px] space-y-2 overflow-auto pr-1" aria-live="polite">{auditEvents.map((event) => <div key={event.eventId} className="rounded-xl border border-border/60 bg-background/45 px-3 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><ScrollText size={15} className="text-primary" /><span className="text-sm font-semibold">{event.action}</span><Badge variant={event.outcome === "SUCCESS" ? "outline" : "destructive"}>{event.outcome}</Badge></div><span className="text-xs text-muted-foreground">{formatDate(event.createdAt)}</span></div><p className="mt-1 text-xs text-muted-foreground">{event.resourceType}:{event.resourceId}{event.actorId ? ` · actor ${event.actorId.slice(0, 8)}…` : ""}</p>{event.reasonCode ? <p className="mt-1 text-xs leading-5">Lý do: {event.reasonCode}</p> : null}</div>)}</div> : <p className="mt-4 rounded-xl border border-dashed border-border/70 px-4 py-8 text-center text-sm text-muted-foreground">Chưa có audit event trong tháng này.</p>}</>}
        </SurfacePanel>

        <SurfacePanel title="Analytics vận hành">
          {!canReadAnalytics ? <p className="text-sm leading-6 text-muted-foreground">Role hiện tại chưa có ANALYTICS_READ. Dữ liệu aggregate chỉ dành cho operator được cấp quyền.</p> : <><div className="flex flex-wrap items-end gap-3"><div className="min-w-[150px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">Từ ngày (UTC)</label><Input className="mt-1" type="date" value={analyticsFrom} onChange={(event) => setAnalyticsFrom(event.target.value)} /></div><div className="min-w-[150px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">Đến ngày (UTC)</label><Input className="mt-1" type="date" value={analyticsTo} onChange={(event) => setAnalyticsTo(event.target.value)} /></div><div className="min-w-[180px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">Event type</label><select value={analyticsType} onChange={(event) => setAnalyticsType(event.target.value)} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm"><option value="ALL">ALL</option><option value="ROOM_CREATED">ROOM_CREATED</option><option value="ROOM_JOINED">ROOM_JOINED</option><option value="MESSAGE_SENT">MESSAGE_SENT</option><option value="POLLS_CREATED">POLLS_CREATED</option><option value="POLL_VOTED">POLL_VOTED</option><option value="CALL_STARTED">CALL_STARTED</option></select></div><p className="pb-2 text-xs text-muted-foreground">Tối đa 31 ngày / 200 điểm hiển thị.</p></div>{analyticsLoading ? <div className="flex justify-center py-8"><LoadingSpinner /></div> : analyticsPoints.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{Object.entries(analyticsPoints.reduce<Record<string, number>>((counts, point) => { counts[point.eventType] = (counts[point.eventType] ?? 0) + 1; return counts; }, {})).map(([type, count]) => <div key={type} className="rounded-xl border border-border/60 bg-background/45 p-3"><div className="flex items-center justify-between gap-2"><span className="flex items-center gap-2 text-xs font-semibold"><BarChart3 size={15} className="text-primary" />{type}</span><span className="text-lg font-black">{count}</span></div><p className="mt-1 text-xs text-muted-foreground">aggregate events trong khoảng đã chọn</p></div>)}</div> : <p className="mt-4 rounded-xl border border-dashed border-border/70 px-4 py-8 text-center text-sm text-muted-foreground">Chưa có analytics trong khoảng này.</p>}</>}
        </SurfacePanel>

        <SurfacePanel title="Reports & sanctions">
          {!canManageReports ? <p className="text-sm leading-6 text-muted-foreground">Role hiện tại chưa có REPORT_MANAGE. Các quyết định moderation chỉ dành cho trust & safety operator.</p> : <>
            <div className="flex flex-wrap items-end gap-3">
              <div className="min-w-[170px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">Ngày report (UTC)</label><Input className="mt-1" type="date" value={reportDay} onChange={(event) => setReportDay(event.target.value)} /></div>
              <div className="min-w-[170px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">Trạng thái</label><select value={reportStatusFilter} onChange={(event) => setReportStatusFilter(event.target.value as AdminReport['status'])} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm"><option value="OPEN">OPEN</option><option value="IN_REVIEW">IN_REVIEW</option><option value="RESOLVED">RESOLVED</option><option value="DISMISSED">DISMISSED</option></select></div>
              <p className="pb-2 text-xs text-muted-foreground">Hàng đợi được phân vùng theo ngày để giữ truy vấn bounded.</p>
            </div>
            <div className="mt-4 space-y-2" aria-live="polite">
              {reportsLoading ? <div className="flex justify-center py-8"><LoadingSpinner /></div> : reports.length ? reports.map((report) => <div key={report.reportId} className="rounded-xl border border-border/60 bg-background/45 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><Badge variant="outline">{report.targetType}</Badge><Badge variant={report.status === "OPEN" ? "destructive" : "secondary"}>{report.status}</Badge><span className="text-xs text-muted-foreground">{report.reportId}</span></div><p className="mt-2 text-sm font-semibold">{report.reasonCode}</p>{report.description ? <p className="mt-1 text-xs leading-5 text-muted-foreground">{report.description}</p> : null}<p className="mt-1 text-xs text-muted-foreground">Reporter: {report.reporterId}{report.targetUserId ? ` · target: ${report.targetUserId}` : ""}</p></div><div className="flex flex-wrap gap-2">{report.targetUserId ? <Button size="sm" variant="outline" onClick={() => { setSanctionTargetUserId(report.targetUserId as string); setSanctions([]); setFeedback("Đã chọn target user cho form sanction."); }}>Chọn target</Button> : null}{report.status === "OPEN" ? <Button size="sm" variant="outline" onClick={() => void handleReportResolution(report, "IN_REVIEW")} loading={moderationMutating}>Nhận xử lý</Button> : null}{report.status === "OPEN" || report.status === "IN_REVIEW" ? <><Button size="sm" onClick={() => void handleReportResolution(report, "RESOLVED")} loading={moderationMutating}>Resolve</Button><Button size="sm" variant="destructive" onClick={() => void handleReportResolution(report, "DISMISSED")} loading={moderationMutating}>Dismiss</Button></> : null}</div></div></div>) : <p className="rounded-xl border border-dashed border-border/70 px-4 py-8 text-center text-sm text-muted-foreground">Không có report trong filter hiện tại.</p>}
            </div>
            <div className="mt-5 rounded-xl border border-border/60 bg-background/45 p-4"><p className="text-sm font-bold">Áp dụng user sanction</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Sanction APP BAN/SUSPEND cập nhật account status; sanction CONVERSATION chỉ áp dụng trong room được chỉ định.</p><div className="mt-3 grid gap-3 md:grid-cols-2"><div><label className="block text-xs font-semibold text-muted-foreground">Target user ID</label><Input className="mt-1" value={sanctionTargetUserId} onChange={(event) => setSanctionTargetUserId(event.target.value)} placeholder="UUID của user" /></div><div><label className="block text-xs font-semibold text-muted-foreground">Scope</label><select value={sanctionScope} onChange={(event) => setSanctionScope(event.target.value as AdminSanction['scope'])} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm"><option value="APP">APP</option><option value="CONVERSATION">CONVERSATION</option></select></div><div><label className="block text-xs font-semibold text-muted-foreground">Loại sanction</label><select value={sanctionType} onChange={(event) => setSanctionType(event.target.value as AdminSanction['sanctionType'])} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm"><option value="BAN">BAN</option><option value="SUSPEND">SUSPEND</option><option value="MUTE">MUTE</option><option value="WARNING">WARNING</option></select></div>{sanctionScope === "CONVERSATION" ? <div><label className="block text-xs font-semibold text-muted-foreground">Conversation ID</label><Input className="mt-1" value={sanctionConversationId} onChange={(event) => setSanctionConversationId(event.target.value)} placeholder="UUID của room" /></div> : null}<div><label className="block text-xs font-semibold text-muted-foreground">Hết hạn (tuỳ chọn)</label><Input className="mt-1" type="datetime-local" value={sanctionExpiresAt} onChange={(event) => setSanctionExpiresAt(event.target.value)} /></div><div><label className="block text-xs font-semibold text-muted-foreground">Reason code</label><Input className="mt-1" value={resolutionCode} onChange={(event) => setResolutionCode(event.target.value)} placeholder="SPAM, ABUSE..." /></div></div><label className="mt-3 block text-xs font-semibold text-muted-foreground">Lý do chi tiết (bắt buộc)</label><Input className="mt-1" value={moderationReason} onChange={(event) => setModerationReason(event.target.value)} placeholder="Ghi rõ căn cứ quyết định" /><Button className="mt-4" onClick={() => void handleImposeSanction()} loading={moderationMutating}>Áp dụng sanction</Button></div>
            {sanctionsLoading ? <div className="mt-5 flex justify-center py-4"><LoadingSpinner /></div> : sanctionTargetUserId && sanctions.length ? <div className="mt-5"><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Sanction của target đang chọn</p><div className="mt-3 space-y-2">{sanctions.map((sanction) => <div key={sanction.sanctionId} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 px-3 py-2"><div><Badge variant={sanction.status === "ACTIVE" ? "destructive" : "outline"}>{sanction.sanctionType} · {sanction.status}</Badge><p className="mt-1 text-xs text-muted-foreground">{sanction.scope} · {sanction.reasonText}</p></div>{sanction.status === "ACTIVE" ? <Button size="sm" variant="outline" onClick={() => void handleRevokeSanction(sanction)} loading={moderationMutating}>Thu hồi</Button> : null}</div>)}</div></div> : null}
          </>}
        </SurfacePanel>

        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.85fr)]">
          <SurfacePanel title="Quyền quản trị hiện tại">
            <div className="flex flex-wrap gap-2">{overview.roles.map((role) => <Badge key={role} variant={roleBadgeVariant(role)}>{role}</Badge>)}</div>
            <p className="mt-5 text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Permission</p>
            ...
            {health ? <p className="mt-5 text-xs text-muted-foreground">Cassandra: <span className="font-semibold text-foreground">{health.cassandra}</span> · kiểm tra lúc {formatDate(health.timestamp)}</p> : <p className="mt-5 text-xs text-muted-foreground">Health endpoint chưa phản hồi; số liệu trên không được thay bằng dữ liệu giả.</p>}
          </SurfacePanel>

          <SurfacePanel title={localizeText("Tìm người dùng")}>
            {!canReadUsers ? <p className="text-sm leading-6 text-muted-foreground">Role hiện tại không có USER_READ. Khu vực này chỉ hiển thị cho operator được cấp quyền đọc user.</p> : null}
            {canReadUsers ? <>
            <form className="flex gap-2" onSubmit={handleSearch}>
              <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="username hoặc tên hiển thị" aria-label="Tìm người dùng quản trị" />
              <Button type="submit" loading={searching} aria-label="Tìm kiếm"><Search size={16} /></Button>
            </form>
            <div className="mt-4 space-y-2" aria-live="polite">
              {users.map((candidate) => <button key={candidate.userId} type="button" onClick={() => void selectUser(candidate)} className="flex w-full items-center justify-between rounded-xl border border-border/60 bg-background/55 px-3 py-2 text-left transition-[color,background-color,border-color,box-shadow,transform,opacity] hover:border-primary/40 hover:bg-primary/5"><span className="flex min-w-0 items-center gap-3"><UserRound size={16} className="shrink-0 text-primary" /><span className="min-w-0"><span className="block truncate text-sm font-semibold">{candidate.displayName}</span><span className="block truncate text-xs text-muted-foreground">@{candidate.userName} · {candidate.status}</span></span></span><span className="text-xs text-muted-foreground">Chọn</span></button>)}
              {!users.length ? <p className="py-5 text-center text-sm text-muted-foreground">Tìm theo username để bắt đầu.</p> : null}
            </div>
            </> : null}
          </SurfacePanel>
        </div>

        {selectedUser && canReadUsers ? <SurfacePanel title={`Quản lý role · @${selectedUser.userName}`}>
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)]">
            <div>
              ...
              <div className="mt-4 rounded-xl border border-border/60 bg-background/45 p-4"><p className="text-sm font-bold">Trạng thái tài khoản toàn cục</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Suspend/ban sẽ chặn phiên mới sau khi security version tăng. Không thể tự khóa tài khoản operator hiện tại.</p><div className="mt-3 flex flex-wrap items-end gap-2"><div className="min-w-[170px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">Status</label><select value={userStatus} onChange={(event) => setUserStatus(event.target.value as 'ACTIVE' | 'SUSPENDED' | 'BANNED')} disabled={!canManageUserStatus} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm"><option value="ACTIVE">ACTIVE</option><option value="SUSPENDED">SUSPENDED</option><option value="BANNED">BANNED</option></select></div><Button variant="outline" onClick={() => void handleUserStatus()} loading={mutating} disabled={!canManageUserStatus || !reason.trim()}>Cập nhật status</Button></div></div>
              <div className="mt-4 rounded-xl border border-border/60 bg-background/45 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-bold">Session & thiết bị</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Danh sách bounded tối đa 100 mục; thu hồi sẽ vô hiệu hóa token/device ở backend.</p></div><Badge variant={canRevokeSessions ? "outline" : "secondary"}>{canRevokeSessions ? "Có quyền thu hồi" : "Chỉ xem"}</Badge></div>{securityLoading ? <div className="flex justify-center py-5"><LoadingSpinner /></div> : <div className="mt-4 space-y-4"><div><p className="text-xs font-bold text-muted-foreground">Thiết bị</p>{userDevices.length ? <div className="mt-2 space-y-2">{userDevices.map((device) => <div key={device.deviceId} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 px-3 py-2"><div><p className="text-sm font-semibold">{device.deviceName ?? device.deviceId}</p><p className="text-xs text-muted-foreground">{device.platform ?? "Chưa có nền tảng"} / {device.appVersion ?? "Chưa có phiên bản"} / {device.active ? "ACTIVE" : "REVOKED"}</p></div>{device.active && canRevokeSessions ? <Button size="sm" variant="outline" onClick={() => void handleRevokeDevice(device)} loading={mutating}>Thu hồi</Button> : null}</div>)}</div> : <p className="mt-2 text-xs text-muted-foreground">Chưa có device projection.</p>}</div><div><p className="text-xs font-bold text-muted-foreground">Refresh sessions</p>{userSessions.length ? <div className="mt-2 space-y-2">{userSessions.map((session) => <div key={session.tokenId} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 px-3 py-2"><div><p className="text-xs font-semibold">{session.tokenId.slice(0, 12)}…</p><p className="text-xs text-muted-foreground">Issued key: {session.issuedAt.slice(0, 12)}… / hết hạn: {formatDate(session.expiresAt)}{session.revokedAt ? " / REVOKED" : ""}</p></div>{!session.revokedAt && canRevokeSessions ? <Button size="sm" variant="outline" onClick={() => void handleRevokeSession(session)} loading={mutating}>Thu hồi</Button> : null}</div>)}</div> : <p className="mt-2 text-xs text-muted-foreground">Chưa có refresh session.</p>}</div></div>}</div>
              <p className="mt-5 text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Role đang gán</p>
              <div className="mt-3 space-y-2">{rolesLoading ? <LoadingSpinner /> : selectedRoles.length ? selectedRoles.map((role) => <div key={role.grantId} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/60 px-3 py-2"><div><Badge variant={roleBadgeVariant(role.roleCode)}>{role.roleCode}</Badge><p className="mt-1 text-xs text-muted-foreground">Cấp lúc {formatDate(role.grantedAt)} · {formatDate(role.expiresAt)}</p></div>{canManageRoles ? <Button size="sm" variant="destructive" onClick={() => void handleRevoke(role)} loading={mutating}>Thu hồi</Button> : null}</div>) : <p className="text-sm text-muted-foreground">Chưa có role ứng dụng.</p>}</div>
            </div>
            <div className="rounded-xl border border-border/60 bg-background/45 p-4">
              {!canManageRoles ? <p className="mb-4 rounded-lg border border-border/60 bg-background/60 p-3 text-xs leading-5 text-muted-foreground">Bạn có thể xem role nhưng không có APP_ROLE_MANAGE để thay đổi.</p> : null}
              <p className="text-sm font-bold">Cấp role</p>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">Chỉ chọn role bạn được phép cấp. SUPER_ADMIN luôn bị backend bảo vệ riêng.</p>
              <label className="mt-4 block text-xs font-semibold text-muted-foreground">Role</label>
              <select value={roleCode} onChange={(event) => setRoleCode(event.target.value)} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm" disabled={!canManageRoles || !availableToGrant.length}><option value="">{availableToGrant.length ? "Chọn role" : "Đã gán toàn bộ role"}</option>{availableToGrant.map((code) => <option key={code} value={code}>{code}</option>)}</select>
              <label className="mt-3 block text-xs font-semibold text-muted-foreground">Lý do (bắt buộc)</label>
              <Input className="mt-1" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Ví dụ: phân công hỗ trợ cộng đồng" disabled={!canManageRoles} />
              <label className="mt-3 block text-xs font-semibold text-muted-foreground">Hết hạn (tuỳ chọn)</label>
              ...
              <Button className="mt-4 w-full" onClick={() => void handleGrant()} loading={mutating} disabled={!canManageRoles || !availableToGrant.length || !roleCode}>Cấp role</Button>
            </div>
          </div>
        </SurfacePanel> : null}
