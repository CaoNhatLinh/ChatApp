        ...
        <div className="grid gap-0 border-y border-border md:grid-cols-3">
          ...
          {!canReadRooms ? <p className="text-sm leading-6 text-muted-foreground">{localizeText("Vai trò hiện tại chưa có ROOM_READ. Đây là quyền cấp ứng dụng, hoàn toàn khác với quyền quản trị trong từng room.")}</p> : <>
            <div className="flex flex-wrap items-end gap-3">
              ...
              <p className="pb-2 text-xs text-muted-foreground">{localizeText("Danh sách được phân vùng theo tháng để không quét Cassandra vô hạn.")}</p>
            </div>
            <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.9fr)]">
              <div className="max-h-[360px] space-y-2 overflow-auto pr-1" aria-live="polite">
                {roomsLoading ? <div className="flex justify-center py-8"><LoadingSpinner /></div> : rooms.length ? rooms.map((room) => <button key={room.conversationId} type="button" onClick={() => void selectRoom(room)} className={`flex w-full items-center justify-between gap-3 rounded-xl border px-3 py-3 text-left transition-[color,background-color,border-color,box-shadow,transform,opacity] ${selectedRoom?.conversationId === room.conversationId ? "border-primary/50 bg-primary/10" : "border-border/60 bg-background/55 hover:border-primary/35"}`}><span className="flex min-w-0 items-center gap-3"><Building2 size={17} className="shrink-0 text-primary" /><span className="min-w-0"><span className="block truncate text-sm font-semibold">{room.name ?? room.conversationId}</span><span className="block truncate text-xs text-muted-foreground">{room.conversationType} · {room.memberCount} {localizeText("thành viên")} · {room.chatMode}</span></span></span><Badge variant={room.deleted ? "destructive" : "outline"}>{room.deleted ? localizeText("Đã lưu trữ") : room.visibility}</Badge></button>) : <p className="rounded-xl border border-dashed border-border/70 px-4 py-8 text-center text-sm text-muted-foreground">{localizeText("Không có room trong tháng này hoặc projection chưa được backfill.")}</p>}
              </div>
              {selectedRoom ? <div className="rounded-xl border border-border/60 bg-background/45 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{selectedRoom.name ?? selectedRoom.conversationId}</p><p className="mt-1 text-xs text-muted-foreground">{selectedRoom.conversationId}</p></div><Badge variant={selectedRoom.deleted ? "destructive" : "secondary"}>{selectedRoom.deleted ? localizeText("Đã lưu trữ") : localizeText("Đang hoạt động")}</Badge></div><dl className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-muted-foreground">{localizeText("Loại room")}</dt><dd className="mt-1 font-semibold">{selectedRoom.conversationType}</dd></div><div><dt className="text-muted-foreground">{localizeText("Thành viên")}</dt><dd className="mt-1 font-semibold">{selectedRoom.memberCount}</dd></div><div><dt className="text-muted-foreground">{localizeText("Chủ room")}</dt><dd className="mt-1 truncate font-semibold">{selectedRoom.ownerId ?? localizeText("Chưa có chủ room")}</dd></div><div><dt className="text-muted-foreground">{localizeText("Ngày tạo")}</dt><dd className="mt-1 font-semibold">{formatDate(selectedRoom.createdAt)}</dd></div></dl><div className="mt-5"><p className="text-xs font-semibold text-muted-foreground">{localizeText("Tổng quan thành viên")}</p><div className="mt-2 flex max-h-20 flex-wrap gap-1.5 overflow-auto">{selectedRoom.members.map((member) => <Badge key={member.userId} variant="outline">{member.userId.slice(0, 8)}…</Badge>)}</div></div><label className="mt-5 block text-xs font-semibold text-muted-foreground">{localizeText("Policy chat toàn cục")}</label><select value={roomPolicy} onChange={(event) => setRoomPolicy(event.target.value)} disabled={!canModerateRooms} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm"><option value="OPEN">{localizeText("Mở")}</option><option value="READ_ONLY">{localizeText("Chỉ đọc")}</option><option value="MANAGERS_ONLY">{localizeText("Chỉ quản lý")}</option></select><label className="mt-3 block text-xs font-semibold text-muted-foreground">{localizeText("Chế độ chậm (giây)")}</label><Input className="mt-1" type="number" min={0} max={86400} value={roomSlowMode} onChange={(event) => setRoomSlowMode(Math.max(0, Number(event.target.value) || 0))} disabled={!canModerateRooms} /><label className="mt-3 block text-xs font-semibold text-muted-foreground">{localizeText("Lý do audit")}</label><Input className="mt-1" value={roomReason} onChange={(event) => setRoomReason(event.target.value)} placeholder={localizeText("Ví dụ: xử lý report spam")} disabled={!canModerateRooms} /><div className="mt-4 flex flex-wrap gap-2"><Button onClick={() => void handleRoomPolicy()} loading={roomMutating} disabled={!canModerateRooms || !roomReason.trim()}>{localizeText("Lưu policy")}</Button><Button variant={selectedRoom.deleted ? "outline" : "destructive"} onClick={() => void handleRoomArchive()} loading={roomMutating} disabled={!canModerateRooms || !roomReason.trim()}><Archive size={16} />{selectedRoom.deleted ? localizeText("Khôi phục") : localizeText("Lưu trữ room")}</Button></div></div> : <div className="flex min-h-[260px] items-center justify-center rounded-xl border border-dashed border-border/70 px-4 text-center text-sm text-muted-foreground">{localizeText("Chọn một room để xem thành viên và thao tác kiểm duyệt toàn cục.")}</div>}
            </div>
          </>}
        </SurfacePanel>
        ...
        <SurfacePanel title={localizeText("Điều tra tin nhắn") }>
          {!canReadAudit ? <p className="text-sm leading-6 text-muted-foreground">{localizeText("Vai trò hiện tại chưa có AUDIT_READ. Nội dung tin nhắn chỉ hiển thị cho người vận hành được cấp quyền điều tra.")}</p> : <>
            ...
            <form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={handleMessageInspection}>
              <div><label className="block text-xs font-semibold text-muted-foreground">{localizeText("ID cuộc trò chuyện")}</label><Input className="mt-1" value={messageConversationId} onChange={(event) => setMessageConversationId(event.target.value)} placeholder="UUID" /></div>
              <div><label className="block text-xs font-semibold text-muted-foreground">{localizeText("ID message")}</label><Input className="mt-1" value={messageId} onChange={(event) => setMessageId(event.target.value)} placeholder="UUID" /></div>
              <div><label className="block text-xs font-semibold text-muted-foreground">{localizeText("Bucket message")}</label><Input className="mt-1" value={messageBucket} onChange={(event) => setMessageBucket(event.target.value)} placeholder="YYYY-MM-DD-HH:NN" /></div>
              <div><label className="block text-xs font-semibold text-muted-foreground">{localizeText("Lý do điều tra (bắt buộc)")}</label><Input className="mt-1" value={messageReason} onChange={(event) => setMessageReason(event.target.value)} placeholder={localizeText("Ví dụ: xử lý báo cáo tin nhắn")}/></div>
        ...
        <SurfacePanel title={localizeText("Báo cáo & chế tài")}>
          {!canManageReports ? <p className="text-sm leading-6 text-muted-foreground">{localizeText("Vai trò hiện tại chưa có REPORT_MANAGE. Quyết định kiểm duyệt chỉ dành cho người vận hành an toàn.")}</p> : <>
            <div className="flex flex-wrap items-end gap-3">
              ...
              <div className="min-w-[170px] flex-1"><label className="block text-xs font-semibold text-muted-foreground">{localizeText("Trạng thái")}</label><select value={reportStatusFilter} onChange={(event) => setReportStatusFilter(event.target.value as AdminReport['status'])} className="mt-1 h-10 w-full rounded-[0.85rem] border border-border/70 bg-background px-3 text-sm"><option value="OPEN">{reportStatusLabel("OPEN")}</option><option value="IN_REVIEW">{reportStatusLabel("IN_REVIEW")}</option><option value="RESOLVED">{reportStatusLabel("RESOLVED")}</option><option value="DISMISSED">{reportStatusLabel("DISMISSED")}</option></select></div>
        ...
        {selectedUser && canReadUsers ? <SurfacePanel title={`${localizeText("Quản lý vai trò")} · @${selectedUser.userName}`}>
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)]">
            ...
            <div className="rounded-xl border border-border/60 bg-background/45 p-4">
              ...
              <label className="mt-4 block text-xs font-semibold text-muted-foreground">{localizeText("Vai trò")}</label>
