  ...
  return (
    <main className="page-shell flex min-h-[100dvh] items-center justify-center px-6 py-12 text-foreground">
      <section className="w-full max-w-xl" aria-labelledby="forbidden-title">
        ...
        <h1 id="forbidden-title" className="max-w-[12ch] text-5xl font-bold leading-[1.02] tracking-[-0.045em] sm:text-6xl">Khu vực này không dành cho tài khoản hiện tại.</h1>
        <p className="mt-5 max-w-lg text-lg leading-8 text-muted-foreground">Quyền ứng dụng được kiểm tra ở máy chủ. Hãy quay lại workspace hoặc đăng nhập bằng tài khoản phù hợp.</p>
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <button type="button" onClick={() => router.push('/app')} className="focus-ring rounded-[var(--radius-md)] bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition-transform hover:-translate-y-px">Về workspace</button>
        </div>
      </section>
    </main>
  );
