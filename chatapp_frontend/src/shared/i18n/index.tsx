...
const isLocale = (value: string | null | undefined): value is AppLocale => value === 'vi' || value === 'en';
...
const localizeValue = <T,>(value: T): T => {
  ...
  if (Array.isArray(value)) {
    const mapped: unknown[] = value.map((item: unknown) => localizeValue(item));
    return mapped as T;
  }
...
export function AppI18nProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const nextLocale = isLocale(stored) ? stored : isLocale(cookie) ? cookie : 'vi';
