      ...
      <div className="relative w-full max-w-4xl h-[85vh] max-h-[850px] bg-card/60 glass rounded-[2.5rem] neo-shadow flex overflow-hidden animate-in zoom-in-95 duration-300 z-10">
        <div className="w-64 border-r border-border/50 bg-background/30 flex flex-col hidden sm:flex">
          ...
          <div className="flex-1 px-3 space-y-2">
            <button onClick={() => setActiveTab('profile')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${activeTab === 'profile' ? 'bg-primary text-primary-foreground neo-shadow' : 'hover:bg-primary/10 text-muted-foreground hover:text-foreground'}`}>
              <UserCircle size={20} />
              <span className="font-bold text-sm">Tài khoản & Hồ sơ</span>
            </button>

            <button onClick={() => setActiveTab('appearance')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${activeTab === 'appearance' ? 'bg-primary text-primary-foreground neo-shadow' : 'hover:bg-primary/10 text-muted-foreground hover:text-foreground'}`}>
              <Palette size={20} />
          ...
          <div className="p-4 border-t border-border/50 mt-auto">
            <button onClick={() => void handleLogout()} className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl hover:bg-destructive text-destructive hover:text-destructive-foreground transition-all neo-shadow active:scale-95 group">
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          <div className="h-16 flex items-center justify-between px-6 border-b border-border/50 bg-background/20 sticky top-0 z-10">
            <h3 className="text-lg font-black uppercase tracking-tight sm:hidden">{activeTab === 'profile' ? 'Tài khoản' : 'Giao diện'}</h3>
            <div className="hidden sm:block"></div>

            <button onClick={onClose} className="p-2 hover:bg-primary/10 rounded-full transition-colors group">
              ...
              {activeTab === 'profile' ? (
                <div className="space-y-8">
                  ...
                  <div className="flex items-center gap-6 p-6 rounded-3xl bg-background/40 border-2 border-border/50">
                    <div className="relative group cursor-pointer" onClick={() => {
                      ...
                      <div className="w-24 h-24 rounded-2xl overflow-hidden border-4 border-background bg-primary/10 flex items-center justify-center neo-shadow transition-transform group-hover:scale-105">
                        {avatarUrl ? <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" /> : <span className="text-4xl font-black text-primary uppercase">{user?.displayName?.charAt(0)}</span>}
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <input type="text" value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="w-full bg-background/50 border-2 border-border/50 rounded-2xl px-5 py-4 focus:border-primary focus:ring-0 transition-all font-bold outline-none" placeholder="Nhập tên hiển thị..." />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" value={nickname} onChange={(event) => setNickname(event.target.value)} className="w-full bg-background/50 border-2 border-border/50 rounded-2xl px-5 py-4 focus:border-primary focus:ring-0 transition-all font-bold outline-none" placeholder="Biệt danh của bạn..." />
                    </div>

                    <div className="space-y-2">
                      <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-2">Avatar URL (Thủ công)</label>
                      <input type="text" value={avatarUrl} onChange={(event) => setAvatarUrl(event.target.value)} className="w-full bg-background/50 border-2 border-border/50 rounded-2xl px-5 py-4 focus:border-primary focus:ring-0 transition-all font-bold outline-none" placeholder="Link ảnh .jpg, .png..." />
                    </div>
                  </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <button onClick={() => setThemePreference('light')} className={`flex flex-col items-center gap-4 p-4 rounded-3xl border-2 transition-all ${themePreference === 'light' ? 'border-primary bg-primary/10' : 'border-border/50 bg-background/40 font-bold text-sm hover:border-primary/50'}`}>
                    ...
                    <button onClick={() => setThemePreference('dark')} className={`flex flex-col items-center gap-4 p-4 rounded-3xl border-2 transition-all ${themePreference === 'dark' ? 'border-primary bg-primary/10' : 'border-border/50 bg-background/40 font-bold text-sm hover:border-primary/50'}`}>
                    ...
                    <button onClick={() => setThemePreference('system')} className={`flex flex-col items-center gap-4 p-4 rounded-3xl border-2 transition-all ${themePreference === 'system' ? 'border-primary bg-primary/10' : 'border-border/50 bg-background/40 font-bold text-sm hover:border-primary/50'}`}>
